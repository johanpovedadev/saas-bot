...
async function handleFinalizeOrder(sock, jid, input, userSession, ctx) {
    ...
    if (validateInput(finalAction, 'confirmation')) {
        ...
        const orderTotal = summary.total + (userSession.order.deliveryCost || 0);        const payload = {
            ...
            nombre: userSession.order.name || '',
            producto: fallbackProductsText,  // ← SINGULAR (backend espera 'producto')
            codigo: fallbackCodes,            // ← SINGULAR (backend espera 'codigo')
            telefono: userSession.order.telefono || '',
            direccion: userSession.order.address || '',
            monto: orderTotal,  // ← Backend espera 'monto' en vez de 'total'
            pago: userSession.order.paymentMethod || '',
            estado: userSession.order.status || 'Por despachar',
            observaciones: `Origen: WhatsApp - Cliente JID: ${jid}`,  // ← Backend usa 'observaciones'
